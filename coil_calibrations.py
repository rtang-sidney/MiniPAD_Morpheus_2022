...
class MNSI_NUC:
    def __init__(self):
        ...
        self.scan_fit_m = [3174, 3174, 3180, 3182, 3184]
...
def lmfit_2d(meas, scan, data_c, data_pol):
    ...
    if scan == SCAN_OI:
        ...
        c_oi = data_c
        fmodel.set_param_hint(VAR_SHIFT_OI, value=0, min=-SHIFT_LIM * 10, max=SHIFT_LIM * 10)
        ...
        fmodel.set_param_hint(VAR_SHIFT_OF, value=0, min=-SHIFT_LIM * 10, max=SHIFT_LIM * 10)
        fmodel.set_param_hint(VAR_SHIFT_II, value=0, vary=False)
        fmodel.set_param_hint(VAR_SHIFT_IF, value=0, vary=False)
    elif scan == SCAN_II:
        ...
        fmodel.set_param_hint(VAR_SHIFT_II, value=0, min=-SHIFT_LIM * 20, max=SHIFT_LIM * 20)
        fmodel.set_param_hint(VAR_SHIFT_IF, value=0, vary=False)
    else:
        ...
        fmodel.set_param_hint(VAR_SHIFT_OF, value=meas.shift_of, vary=False)
        fmodel.set_param_hint(VAR_SHIFT_II, value=0, vary=False)
        fmodel.set_param_hint(VAR_SHIFT_IF, value=0, min=-SHIFT_LIM * 20, max=SHIFT_LIM * 20)
...
def lm_fit_4d(meas, monitor, data_oi, data_of, data_ii, data_if, data_pol):
    ...
    fmodel.set_param_hint(VAR_SHIFT_OI, value=meas.shift_oi, min=meas.shift_oi - SHIFT_LIM,
                          max=meas.shift_oi + SHIFT_LIM)
    fmodel.set_param_hint(VAR_SHIFT_OF, value=meas.shift_of, min=meas.shift_of - SHIFT_LIM,
                          max=meas.shift_of + SHIFT_LIM)
    fmodel.set_param_hint(VAR_SHIFT_II, value=meas.shift_ii, min=meas.shift_ii - SHIFT_LIM,
                          max=meas.shift_ii + SHIFT_LIM)
    fmodel.set_param_hint(VAR_SHIFT_IF, value=meas.shift_if, min=meas.shift_if - SHIFT_LIM,
                          max=meas.shift_if + SHIFT_LIM)
    fmodel.set_param_hint(VAR_DELTA_I, value=meas.delta_i, vary=False)
    fmodel.set_param_hint(VAR_DELTA_F, value=meas.delta_f, vary=False)
...
def lm_fit_new(fmodel, meas, scan, monitor, data_oi, data_of, data_ii, data_if, data_pol, pre_oi=0.75, pre_of=-0.75,
               pre_ii=1.4, pre_if=1.0, shift_oi=0.0, shift_of=0.0, shift_ii=0.0, shift_if=0.0, p_init=0.8):
    if scan == SCAN_OI:
    ...
    if scan == SCAN_FIT:
        fmodel.independent_vars = [VAR_C_OI, VAR_C_OF, VAR_C_II, VAR_C_IF]
        fmodel.set_param_hint(VAR_PRE_OI, value=pre_oi, min=pre_oi - abs(pre_oi) * 0.5, max=pre_oi + abs(pre_oi) * 0.5)
        fmodel.set_param_hint(VAR_SHIFT_OI, value=shift_oi, min=shift_oi - SHIFT_LIM, max=shift_oi + SHIFT_LIM)
    ...
    params = fmodel.make_params()
    print(data_oi, data_of, data_ii, data_if, data_pol)
    print(fmodel.independent_vars)
    print(params)
    if scan == SCAN_OI:
        result = fmodel.fit(data_pol, params, c_oi=data_oi)
        print(result.fit_report())
        fmodel = None
        return result.params[VAR_PRE_OI].value, result.params[VAR_SHIFT_OI].value, result.params[VAR_P_INIT].value
    elif scan == SCAN_OF:
        result = fmodel.fit(data_pol, params, c_of=data_of)
        print(result.fit_report())
        fmodel = None
        ...
        # print("ii", params)
        result = fmodel.fit(data_pol, params, c_ii=data_ii)
        print(result.fit_report())
        fmodel = None
        return result.params[VAR_PRE_OI].value, result.params[VAR_PRE_OF].value, result.params[VAR_PRE_II].value, \
        ...
        result = fmodel.fit(data_pol, params, c_if=data_if)
        print(result.fit_report())
        fmodel = None
        return result.params[VAR_PRE_OI].value, result.params[VAR_PRE_OF].value, result.params[VAR_PRE_II].value, \
...
def coils_fitting(meas, scan, pre_oi=None, pre_of=None, pre_ii=None, pre_if=None, shift_oi=None, shift_of=None,
    ...
    if scan == SCAN_ALL:
        ...
        print(VAR_C_OI)
        fmodel_oi = Model(coil_system, independent_vars=[VAR_C_OI])
        print(fmodel_oi)
        pre_oi, shift_oi, p_init = lm_fit_new(fmodel_oi, meas, scan, scan_monitor, scan_coils[0], scan_coils[1],
        ...
        fmodel_of = Model(coil_system, independent_vars=[VAR_C_OF])
        pre_oi, pre_of, shift_oi, shift_of, p_init = lm_fit_new(fmodel_of, meas, scan, scan_monitor, scan_coils[0],
                                                                scan_coils[1],
        ...
        fmodel_ii = Model(coil_system, independent_vars=[VAR_C_II])
        pre_oi, pre_of, pre_ii, shift_oi, shift_of, shift_ii, p_init = lm_fit_new(fmodel_ii, meas, scan, scan_monitor,
                                                                                  scan_coils[0], scan_coils[1],
                                                                                  scan_coils[2], scan_coils[3], pols,
                                                                                  pre_oi=pre_oi, pre_of=pre_of,
                                                                                  shift_oi=shift_oi, shift_of=shift_of,
                                                                                  p_init=p_init)
        # plot_1d2(meas, scan, numbers_p, numbers_m, scan_coil, counts_p, counts_m, pols, pre, shift, p_init)
        return pre_oi, pre_of, pre_ii, shift_oi, shift_of, shift_ii, p_init
    elif scan == SCAN_IF:
        fmodel_if = Model(coil_system, independent_vars=[VAR_C_IF])
        pre_oi, pre_of, pre_ii, pre_if, shift_oi, shift_of, shift_ii, shift_if, p_init = lm_fit_new(fmodel_if, meas,
                                                                                                    scan, scan_monitor,
                                                                                                    scan_coils[0],
                                                                                                    scan_coils[1],
                                                                                                    scan_coils[2],
...
coils_fitting(meas=measure, scan=SCAN_FIT, pre_oi=pre_oi, pre_of=pre_of, pre_ii=pre_ii, pre_if=pre_if,
